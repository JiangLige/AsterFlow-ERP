...
- Redis：Dashboard 缓存、商品详情缓存、空值缓存、商品布隆过滤器、热点 Key 互斥重建、认证会话、幂等提交和限流都有可选 Redis 实现。
- Spring AI：已接入 Spring AI 2.0.0、OpenAI ChatClient、只读工具调用和结构化库存建议响应。

仍未完成或不能夸大的部分：

- 缓存雪崩治理、Redis 集群高可用仍是后续优化。
...
- 商品布隆过滤器支持 `local|redis`，启动时会把已有商品 ID 预热进过滤器。
- 商品详情缓存未命中时使用带 TTL 的重建锁；Redis 模式使用分布式锁，local 模式使用进程内锁。
...
- Redis 集群、高可用和真实压测。
- 缓存 TTL 随机抖动、逻辑过期等雪崩治理。

面试讲法：

> Redis 在这个项目里不是业务数据源，而是缓存和协调组件。MySQL 负责订单、库存和流水的最终一致性；Redis 用于 Dashboard 缓存、商品详情缓存、空值缓存、布隆过滤器、热点 Key 重建锁、登录会话、幂等提交和限流。为了方便本地开发，我保留了 local 模式，生产环境再按配置切换到 Redis。
...
- 写 MySQL 后删除缓存。
- 读请求未命中时重新查数据库并回写缓存。
- 热点商品未命中时先竞争带 TTL 的重建锁，其他请求短暂等待缓存回填。
