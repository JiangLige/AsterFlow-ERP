...
- Redis：Dashboard 缓存、商品详情缓存、空值缓存、商品布隆过滤器、认证会话、幂等提交和限流都有可选 Redis 实现。
...
> Redis 在这个项目里不是业务数据源，而是缓存和协调组件。MySQL 负责订单、库存和流水的最终一致性；Redis 用于 Dashboard 缓存、商品详情缓存、空值缓存、商品布隆过滤器、登录会话、幂等提交和限流。为了方便本地开发，我保留了 local 模式，生产环境再按配置切换到 Redis。
