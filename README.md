...
- 缓存与可靠性：local/Redis Dashboard 缓存、商品详情缓存、空值缓存、幂等提交、Redis 限流
...
后端测试覆盖采购、销售、库存、认证会话、API 安全边界、商品详情缓存、Redis 限流降级、AI 工具 DTO 和 OpenAPI 文档。
...
- 布隆过滤器、热点 Key 互斥重建仍属于后续增强；商品详情缓存和空值缓存已经具备 local/Redis 实现。
