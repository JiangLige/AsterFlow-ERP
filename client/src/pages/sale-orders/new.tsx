...
export default function SaleOrderCreatePage() {
  ...
  useEffect(() => {
    async function loadOptions() {
      ...
      } catch (requestError) { setError(requestError instanceof Error ? requestError.message : '客户或商品加载失败'); }
    }
    loadOptions();
  }, []);
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    for (const [productId, quantity] of quantityByProductId) {
      ...
      if (product && quantity > product.stock) { setError(`商品库存不足：${product.name}，当前库存 ${product.stock}，本次销售 ${quantity}`); return; }
    }
    setSubmitting(true);
    try {
    ...
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : '新增销售单失败'); }
