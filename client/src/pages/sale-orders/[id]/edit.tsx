...
export default function SaleOrderEditPage() {
  ...
  useEffect(() => {
    ...
    async function loadData() {
      ...
      } catch (requestError) { setError(requestError instanceof Error ? requestError.message : '加载销售单失败'); }
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (status !== 'DRAFT') { setError('只有草稿销售单可以修改'); return; }
    if (!customerId || Number(customerId) <= 0) { setError('请选择客户'); return; }
    ...
    if (invalidItem) { setError('商品、数量、销售价都必须填写；数量必须是正整数，销售价必须大于0'); return; }
    ...
    for (const [productId, quantity] of quantityByProductId) { const product = products.find((item) => item.id === productId); if (product && quantity > product.stock) { setError(`商品库存不足：${product.name}，当前库存 ${product.stock}，本次销售 ${quantity}`); return; } }
    setSubmitting(true);
    try {
    ...
    } catch (requestError) { setError(requestError instanceof Error ? requestError.message : '修改销售单失败'); }
  ...
  return (
    <Layout>
      <main className="aster-form-page aster-order-form-page">
        ...
        {error ? <InlineNotification hideCloseButton kind="error" lowContrast role="alert" subtitle={error} title="销售单保存失败" /> : null}
        ...
        {!loading ? <Form className="aster-form-grid" onSubmit={handleSubmit}>
          ...
          <fieldset className="aster-form-field--full aster-order-items-fieldset" disabled={status !== 'DRAFT'}>
            <OrderItemsEditor items={items} onAdd={addItem} onChange={(index, field, value) => field === 'productId' ? updateProduct(index, value) : updateItem(index, field, value)} onRemove={removeItem} priceLabel="销售价" products={products} />
          </fieldset>
