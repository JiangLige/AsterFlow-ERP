...
- Final review machine-readable browser gate:
  `docs/superpowers/qa/carbon-precision/final-review-browser-qa-log.json`.
...
| P0 | Authenticated form and order-detail pages nested a second `main` landmark inside the shell landmark. | Replaced authenticated page and `OrderDetailLayout` wrappers with `div`; rendered regressions and final Chrome checks prove representative form and detail routes each contain exactly one main landmark. |
| P1 | Inventory warnings omitted the shortage amount and did not provide row-level operational destinations. | Restored `库存缺口`, stock-adjustment, and replenishment destinations using existing routes; rendered regressions and final Chrome checks confirm the calculated gap and both hrefs. |
| P1 | Stock records omitted the existing `beforeStock` field. | Restored the `变更前库存` column; rendered regressions and final Chrome checks confirm both before and after values. |
| P1 | Dashboard Recharts tooltip and legend props created runtime inline styles. | Replaced `contentStyle` and `wrapperStyle` with SCSS-backed custom renderers; preflight rejects those props, and final Chrome checks confirm both custom renderers appear. |
| P2 | Button/input transitions animated visual properties outside the approved motion scope, and authenticated titles could exceed 32px. | Restricted project and Carbon control transitions to `transform` and `opacity`, expanded the stylesheet contract, capped `.aster-page-header h1` at 2rem, and verified computed styles plus a 32px title in Chrome. |
